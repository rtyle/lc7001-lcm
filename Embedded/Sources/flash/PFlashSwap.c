...
0.1.2       11.16.2011      FPT Team      Update the checking for transition condition
                                          ...
                                          API in driver
0.1.3       02.20.2012      FPT Team      Changed function prototype to
                                          require one more parameter - 
                                          PFLASH_SWAP_CALLBACK.
*************************************************************************/
...
/************************************************************************
*
*  Function Name    : _PFlashEraseSector
*  Description      : Erase a flash sector at the given address
*                     
*                     
...
/************************************************************************
*
*  Function Name    : _ReportSwapState
*  Description      : Report the current swap state via callback
*                     
*                     
*  Arguments        : PFLASH_SSD_CONFIG, UINT32, UINT8*, PFLASH_SWAP_CALLBACK, 
*                     BOOL*, pFLASHCOMMANDSEQUENCE
*                     
*  Return Value     : UINT32
*
*************************************************************************/
static UINT32 _ReportSwapState(PFLASH_SSD_CONFIG PSSDConfig, \
                               UINT32 flashAddress, \
                  ...
                  pFLASHCOMMANDSEQUENCE FlashCommandSequence)
{
    ...
    returnCode = _ReportSwapState(PSSDConfig, flashAddress, \
        &currentSwapMode, pSwapCallback, &swapContinue, FlashCommandSequence);

    if (FTFx_OK != returnCode || !swapContinue)
    {
        goto EXIT;
    }
    ...
    if (FTFx_SWAP_UNINIT == currentSwapMode)
    {
        ...
        while (((currentSwapMode = REG_READ(PSSDConfig->ftfxRegBase + FTFx_SSD_FCCOB5_OFFSET)) != FTFx_SWAP_READY) &&
               ((currentSwapMode = REG_READ(PSSDConfig->ftfxRegBase + FTFx_SSD_FCCOB5_OFFSET)) != FTFx_SWAP_UPDATE) &&
               ((currentSwapMode = REG_READ(PSSDConfig->ftfxRegBase + FTFx_SSD_FCCOB5_OFFSET)) != FTFx_SWAP_UPDATE_ERASED));
    ...
    if (FTFx_SWAP_READY == currentSwapMode)
    {
        ...
        while (((currentSwapMode = REG_READ(PSSDConfig->ftfxRegBase + FTFx_SSD_FCCOB5_OFFSET)) != FTFx_SWAP_UPDATE) &&
               ((currentSwapMode = REG_READ(PSSDConfig->ftfxRegBase + FTFx_SSD_FCCOB5_OFFSET)) != FTFx_SWAP_UPDATE_ERASED));
...
/* Report swap state via callback */
static UINT32 _ReportSwapState(PFLASH_SSD_CONFIG PSSDConfig, \
                               UINT32 flashAddress, \
                               UINT8* currentSwapMode, \
                               PFLASH_SWAP_CALLBACK pSwapCallback, \
                               ...
                               pFLASHCOMMANDSEQUENCE FlashCommandSequence)
{
    UINT32 returnCode = FTFx_OK;

    /* Get current swap state using REPORT command */
    ...
    *currentSwapMode = REG_READ(PSSDConfig->ftfxRegBase + FTFx_SSD_FCCOB5_OFFSET);
    ...
    //MPL
    //if (NULL_CALLBACK != pSwapCallback)
