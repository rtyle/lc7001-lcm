...
#define BOOST_TEST_MODULE SSLExternalNetworkBridgeTest
   ...
   public:
         ...
         bridge(nullptr)
      {
         ...
         if(db)
         {
            ...
            // Create the bridge
            logInfo("Creating the bridge");
            bridge = std::make_shared<ExternalNetworkBridge>(APP_PORT, LCM_PORT, REGISTRATION_PORT, DB_NAME, serverCertFilename, serverKeyFilename, clientCertFilename);
            bridge->start();
         }
         else
         {
...
BOOST_FIXTURE_TEST_CASE(Test_LCMClientConnected, BridgeFixture)
{
   ...
   BOOST_REQUIRE_EQUAL(true, small.openConnection(SERVER_HOST, LCM_PORT, clientCertFilename, clientKeyFilename, serverCertFilename));
   BOOST_REQUIRE_EQUAL(true, random.openConnection(SERVER_HOST, LCM_PORT, clientCertFilename, clientKeyFilename, serverCertFilename));
   BOOST_REQUIRE_EQUAL(true, large.openConnection(SERVER_HOST, LCM_PORT, clientCertFilename, clientKeyFilename, serverCertFilename));
...
BOOST_FIXTURE_TEST_CASE(Test_RegistrationClientConnected, BridgeFixture)
{
   ...
   BOOST_REQUIRE_EQUAL(true, registrationClient.openConnection(SERVER_HOST, REGISTRATION_PORT, clientCertFilename, clientKeyFilename, serverCertFilename)); 
...
BOOST_FIXTURE_TEST_CASE(Test_AppClientConnected_DBEmpty, BridgeFixture)
{
   ...
   BOOST_REQUIRE_EQUAL(true, appClient.openConnection(SERVER_HOST, APP_PORT, clientCertFilename, clientKeyFilename, serverCertFilename));
...
BOOST_FIXTURE_TEST_CASE(Test_LCMClosedFirst, BridgeFixture)
{
   ...
   BOOST_REQUIRE_EQUAL(true, lcmClient.openConnection(SERVER_HOST, LCM_PORT, clientCertFilename, clientKeyFilename, serverCertFilename));
...
BOOST_FIXTURE_TEST_CASE(Test_EndToEnd, BridgeFixture)
{
   ...
   BOOST_REQUIRE_EQUAL(true, waitForJsonMessage(sendObject, std::chrono::milliseconds(10000)));
