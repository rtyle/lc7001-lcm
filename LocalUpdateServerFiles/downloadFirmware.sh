...
set update_server_hostname [lindex $argv 2];
set update_server_branch [lindex $argv 3];
...
while { $forever == 1 } {
  ...
  expect {
    ...
    # Wait for MAC address before sending firmware download command
    -re "0026EC......"
    {
      sleep 2

      send_user "\n*** Sending command to trigger firmware download\n\n"
            
      send "{ \"ID\": 2, \"Service\": \"FirmwareUpdate\", \"Host\": \"$update_server_hostname\", \"Branch\": \"$update_server_branch\"}";
      send "\x00";
      send "\x04";
