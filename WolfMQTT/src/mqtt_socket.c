...
2019-04-26: Ensure WolfMQTT is nonblocking and has timeout capability
    ...
    void *ptr)
{
    ...
    rc = client->net->read_cb(client->net->context, (byte*)buf, sz,
    ...
    void *ptr)
{
    ...
    rc = client->net->write_cb(client->net->context, (byte*)buf, sz,
        client->tls.timeout_ms);
...
int MqttSocket_Init(MqttClient *client, MqttNet *net)
{
    ...
    if (client) {
        ...
        if (net && net->connect_cb && net->read_cb && net->write_cb && net->disconnect_cb) {
    ...
    int timeout_ms)
{
    ...
    {
        rc = client->net->write_cb(client->net->context, buf, buf_len,
            timeout_ms);
    }
    ...
    int timeout_ms)
{
    ...
    /* Validate arguments */
    if (client == NULL || client->net == NULL || client->net->write_cb == NULL ||
        buf == NULL || buf_len <= 0) {
...
#endif /* ENABLE_MQTT_TLS */
    {
        rc = client->net->read_cb(client->net->context, buf, buf_len, timeout_ms);
    }
...
int MqttSocket_Read(MqttClient *client, byte* buf, int buf_len, int timeout_ms)
{
    ...
    /* Validate arguments */
    if (client == NULL || client->net == NULL || client->net->read_cb == NULL ||
        buf == NULL || buf_len <= 0) {
        return MQTT_CODE_ERROR_BAD_ARG;
    }
    ...
    int timeout_ms, int use_tls, MqttTlsCb cb)
{
    ...
    if ((client->flags & MQTT_CLIENT_FLAG_IS_CONNECTED) == 0) {
        ...
        /* Connect to host */
        rc = client->net->connect_cb(client->net->context, host, port, timeout_ms);
        if (rc < 0) {
            return rc;
        }
        client->flags |= MQTT_CLIENT_FLAG_IS_CONNECTED;
...
int MqttSocket_Disconnect(MqttClient *client)
{
    ...
    if (client) {
        ...
        if (client->net && client->net->disconnect_cb) {
            rc = client->net->disconnect_cb(client->net->context);
        }
        client->flags &= ~MQTT_CLIENT_FLAG_IS_CONNECTED;
    }
#ifdef WOLFMQTT_DEBUG_SOCKET
